...
Neo4JStore.prototype.authenticate = function (token, uid, callback) {
  ...
  let query = `MATCH (t:${this._nodeLabel} {uid: t.uid}) WHERE t.ttl > {now} RETURN t`
  let params = {uid: uid, now: new Date().getTime()}
  this._session.run(query, params)
    .then(queryResult => {
...
Neo4JStore.prototype.storeOrUpdate = function (token, uid, msToLive, originUrl, callback) {
  ...
  bcrypt.hash(token, 10, function (error, hashedToken) {
    ...
    self._findUser(uid, (error, user) => {
      if (error || !user) {
        query = `
            CREATE (t:${self._nodeLabel} {user}) RETURN t
          `
        params = {user: body}
      } else {
        query = `
            MATCH (t:${self._nodeLabel}) 
            WHERE t.uid = {uid}
...
Neo4JStore.prototype.invalidateUser = function (uid, callback) {
  ...
  const query = `MATCH (t:${this._nodeLabel} {uid: {uid}}) DETACH DELETE t`
  this._session.run(query, {uid: uid})
  .then(() => {
    callback()
  })
  .catch((error) => {
...
Neo4JStore.prototype.clear = function (callback) {
  ...
  const query = `MATCH (t:${this._nodeLabel}) DETACH DELETE t`
...
Neo4JStore.prototype.length = function (callback) {
  ...
  const query = `MATCH (t:${this._nodeLabel}) RETURN t`
...
Neo4JStore.prototype._findUser = function (uid, callback) {
  ...
  let query = `MATCH (t:${this._nodeLabel} {uid: {uid}}) RETURN t`
